...
class EmployeeQueryServiceTest {
  ...
  void getEmployees_Success() {
    ...
    // Mocking TypedQuery for both count and data
    ...
    verify(em, times(1)).createQuery(contains("SELECT COUNT(e)"), eq(Long.class));
    verify(em, times(1)).createQuery(contains("SELECT e FROM Employee e"), eq(Employee.class));
